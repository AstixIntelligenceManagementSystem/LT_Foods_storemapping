...
*/
                            ...
                            else
                            {
                                ...
                                String StoreName= tokens.nextToken().trim();
                                String StoreType= tokens.nextToken().trim();
                                String StoreLatitude= tokens.nextToken().trim();
                                String StoreLongitude= tokens.nextToken().trim();
                                String LastVisitDate= tokens.nextToken().trim();
                                String LastTransactionDate= tokens.nextToken().trim();
                                String dateVAL= tokens.nextToken().trim();
                                String AutoIdStore= tokens.nextToken().trim();
                                String Sstat= tokens.nextToken().trim();
                                String IsClose= tokens.nextToken().trim();
                                String IsNextDat= tokens.nextToken().trim();
                                String RouteID= tokens.nextToken().trim();
                                int flgHasQuoteNew= Integer.parseInt(tokens.nextToken().trim());
                                int flgAllowQuotationNew= Integer.parseInt(tokens.nextToken().trim());
                                int flgSubmitFromQuotationNew= Integer.parseInt(tokens.nextToken().trim());
...
*/
                                ...
                                else
                                {
                                    ...
                                    int flgHasQuoteNew= Integer.parseInt(tokens.nextToken().trim());
                                    int flgAllowQuotationNew= Integer.parseInt(tokens.nextToken().trim());
                                    int flgSubmitFromQuotatioNew= Integer.parseInt(tokens.nextToken().trim());
                                    helperDb.fnDeletesaveNewOutletFromOutletMstr(selStoreID);
                                    String allValuesOfPaymentStageID=helperDb.fngettblNewStoreSalesQuotePaymentDetails(selStoreID);
                                    helperDb.open();
/*                                    helperDb.fndeleteNewStoreSalesQuotePaymentDetails(selStoreID);
    ...
    private class GetAddingStoreInfo extends AsyncTask<Void, Void, Void>
    {
        ...
        protected void onPreExecute()
        {
           ...
           // SimpleDateFormat df = new SimpleDateFormat("dd-MM-yyyy HH:mm:ss");
            VisitEndTS = getDateAndTimeInSecond();//df.format(dateobj);
                    ...
                    if(mm==1)
                    {
                      ...
                      //  SimpleDateFormat df = new SimpleDateFormat("dd-MM-yyyy HH:mm:ss.SSS");
                       ...
                       // getRouteservice = getRouteservice.getNewStoreInfoDynamic(getApplicationContext(),ArrAnsAndQuestionTable,ArrOutletGeneralInfoTable,imei,ArrOutletPaymentDetails);
                        ...
                        if(!getRouteservice.director.trim().equals("1"))
                        {
                ...
                for(int mm = 1; mm<2; mm++)
                {
                    if(mm==1)
                    {
                      ...
                      //  SimpleDateFormat currDateFormatNew = new SimpleDateFormat("dd-MMM-yyyy", Locale.ENGLISH);
                        ...
                        String currSysDateNew = getDateInMonthTextFormat();//currDateFormatNew.format(currDateNew);
    ...
    private class FullSyncDataNow extends AsyncTask<Void, Void, Void>
    {
        ...
        protected Void doInBackground(Void... params)
        {
           ...
           /* long  syncTIMESTAMP = System.currentTimeMillis();
            ...
            newfullFileName=imei+"."+ getDateAndTimeInSecondForMakingXML();//df1.format(dateobj);
        ...
        protected void onPostExecute(Void result) {
            ...
            try
            {
                ...
                if(isOnline())
                {
                    ...
                    syncIntent.putExtra("xmlPathForSync", Environment.getExternalStorageDirectory() + "/" + CommonInfo.LTFoodOrderXMLFolder + "/" + newfullFileName + ".xml");
